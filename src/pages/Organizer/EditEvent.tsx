...
export default function EditEvent() {
  ...
  const [typeOfEvent, setTypeOfEvent] = React.useState<EventType>(selectEventState?.type || "Regular");
  const [eventName, setEventName] = React.useState(selectEventState?.name || "");
  const [description, setDescription] = React.useState(selectEventState?.description || "");
  //@ts-ignore
  const [date, setDate] = React.useState<Date | undefined>(selectEventState?.date?.toDate() || new Date());
  ...
  const [markerPosition, setMarkerPosition] = React.useState(selectEventState?.location || initialRegion);

  React.useEffect(() => {
    if(selectEventState?.location) return;
    (async () => {
  ...
  const handleGoNext = () => {
    ...
    navigation.navigate("Organizer/EditEventImage" as never);
  };
